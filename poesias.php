...
<title>Ruy Guerra - poesias</title>
<body>
...
$ordem = $_GET["ordem"];
if ($ordem == 1) {
	$sql = $mysqli->query("select id,titulo,autor,dia,mes,ano,local,cidade,pais,fonte,tipo from rg where categoria='poesia' order by titulo,ano desc,mes desc,dia desc");
} elseif ($ordem == 2) {
	$sql = $mysqli->query("select id,titulo,autor,dia,mes,ano,local,cidade,pais,fonte,tipo from rg where categoria='poesia' order by ano,mes,dia,titulo");
}	else {
	$sql = $mysqli->query("select id,titulo,autor,dia,mes,ano,local,cidade,pais,fonte from rg where categoria='poesia' order by ano desc,mes desc,dia desc");
}
...
<section class="conteudo">
	<br />
	<img class="ruy" src="images/ruy_guerra.png" alt="Ruy Guerra" title="Ruy Guerra" /><img class="tituloCategoriaV" src="images/poesiasv.jpg" alt="poesias" title="poesias" />
	<br class="all" /><br /><br /><form method="post" action="busca.php?idioma=$idioma">
	<p class="ordem">Ordenar por: <a class="ordem" href="poesias.php?ordem=1&idioma=$idioma">título</a> &nbsp; <a class="ordem" href="poesias.php?ordem=2&idioma=$idioma">+antigo</a> &nbsp; <a class="ordem" href="poesias.php?ordem=3&idioma=$idioma">+recente</a></p>
...
while ($f = $sql->fetch_array(MYSQLI_NUM)) {
	...
	$titulo = troca($f[1]);
	echo "<p class=\"lista\"><a href=\"poesia.php?id=$f[0]&idioma=$idioma\">$titulo</a> $data<br />";
